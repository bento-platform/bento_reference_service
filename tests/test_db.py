...
async def test_create_genome(db: Database, db_cleanup):
    # SARS-CoV-2
    await db.create_genome(Genome(**TEST_GENOME_OF_FILE_URIS), return_external_resource_uris=False)
...
async def test_mark_running_as_error(db: Database, db_cleanup):
    g = await db.create_genome(Genome(**TEST_GENOME_OF_FILE_URIS), return_external_resource_uris=False)
